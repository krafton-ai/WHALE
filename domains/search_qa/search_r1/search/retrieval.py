...
class DenseRetriever(BaseRetriever):
    ...
    def __init__(self, config: dict):
        ...
        cpu_n_vectors = cpu_index.ntotal
        cpu_dim = cpu_index.d
        if config.faiss_gpu:
            co = faiss.GpuMultipleClonerOptions()
            co.useFloat16 = True   # Halves on-GPU footprint (fp32 → fp16); ~30.5 GB for wiki-18.
            co.shard = True        # When CUDA_VISIBLE_DEVICES is multi-GPU, shard the index;
                                   # with a single visible GPU it's a no-op (full index local).
            self.index = faiss.index_cpu_to_all_gpus(cpu_index, co=co)
            # Verify what landed on the GPU side. Helpful for catching the
            # case where useFloat16 is silently ignored by an unsupported
            # FAISS build (we'd see a 2× larger GPU footprint than expected).
            est_bytes = cpu_n_vectors * cpu_dim * 2  # fp16 → 2 B/element
            print(
                f"[retrieval] FAISS uploaded to GPU: "
                f"n_vectors={cpu_n_vectors}, dim={cpu_dim}, "
                f"useFloat16={co.useFloat16}, shard={co.shard}, "
                f"expected_gpu_footprint≈{est_bytes / 1e9:.2f} GB",
                flush=True,
            )
        else:
