...
Copied (with light edits) from the Meta-Harness paper's reference examples so
this experiment can drive Claude Code the same way `text_classification` /
`terminal_bench_2` do:

- Calls the Claude Code CLI via subprocess
- Parses stream-json output
- Tracks tool calls, file reads/writes, and token usage
- Writes per-session logs (meta.json, response.md, events.jsonl, tools/) so
  each proposer iteration is fully auditable

Works independently of the local Claude Code setup (skills/plugins are NOT
inherited).
"""
...
def run(
    ...
    """Run `claude -p` and return a parsed SessionResult. Logs to log_dir.

    Uses the local Claude Code auth (subscription or ANTHROPIC_API_KEY).
    """
    if log_dir is None:
