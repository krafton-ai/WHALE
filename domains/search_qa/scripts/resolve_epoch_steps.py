#!/usr/bin/env python3
"""Resolve epoch-based training frequencies to step counts."""
...
def main() -> int:
    ...
    # RayPPOTrainer builds the train StatefulDataLoader with drop_last=True, so
    # epoch-derived step counts must use floor division to match actual runs.
    steps_per_epoch = max(1, rows // args.train_batch_size)
    ...
    if round_steps is not None:
        print(f"ROUND_STEPS={round_steps}")
        print(f"ROUND_GRPO_STEPS={round_steps}")
        print(f"ROUND_RSFT_STEPS={round_steps}")
    return 0
