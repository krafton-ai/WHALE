...
class MultiTurnSFTDataset(Dataset):
    ...
    def _process_prefix_messages(
        self,
        messages: list[dict[str, Any]],
        tools: Optional[list[dict[str, Any]]] = None,
        enable_thinking: Optional[bool] = None,
    ) -> tuple[list[int], list[int], list[int]]:
        """Tokenize a non-training prefix such as system+first user together.

        Qwen3.5's chat template rejects a standalone system message, while
        candidate harnesses can define SYSTEM_PROMPT. Keeping the first
        system+user prefix together preserves the inference prompt format and
        masks the whole prefix out of the SFT loss.
        """
        processor = self.processor if self.processor is not None else self.tokenizer
        apply_chat_template_kwargs = {**self.apply_chat_template_kwargs}
        if enable_thinking is not None:
    ...
    def __getitem__(self, item):
        ...
        while i < len(messages):
            if (
                ...
                _input_ids, _loss_mask, _attention_mask, _inputs = self._process_prefix_messages(
                    messages=messages[:2],
                    tools=tools,
