...
# Lazy tokenizer cache — one entry per (model_name) per worker process.
# Used to enforce a per-doc token cap so a single long retrieval result
# cannot blow up the LLM context (and the vLLM KV cache by extension).
_TOKENIZER_CACHE: dict[str, Any] = {}


def _get_tokenizer(name: str):
    """Lazy-load a tokenizer; cached per process for reuse across calls."""
...
def _truncate_to_tokens(text: str, max_tokens: int, tokenizer_name: str) -> str:
    """Truncate `text` to its first `max_tokens` tokens under `tokenizer_name`.

    No-op when tokenizer_name or max_tokens is falsy, or when the text already
    fits. Failures (e.g. tokenizer download error) fall back to the original
    text rather than crashing the rollout.
    """
    ...
    try:
        ...
        logger.warning(f"_truncate_to_tokens fell back due to: {e}")
        return text
...
def _passages2string(retrieval_result, max_doc_tokens=None, tokenizer_name=None):
    """Convert retrieval results to formatted string.

    If `max_doc_tokens` is set (and `tokenizer_name` provided), each doc's full
    `contents` field is truncated to its first `max_doc_tokens` tokens BEFORE
    being split into title/body. Most wiki-18 passages are 130–180 tokens, so
    a cap of ~200 only trims rare long outliers and keeps tool_response size
    bounded → predictable LLM context, no vLLM OOM from runaway docs.
    """
    format_reference = ""
    for idx, doc_item in enumerate(retrieval_result):
        content = doc_item["document"]["contents"]
        if max_doc_tokens:
            content = _truncate_to_tokens(content, max_doc_tokens, tokenizer_name)
        title = content.split("\n")[0]
        text = "\n".join(content.split("\n")[1:])
