...
class DataParallelPPOActor(BasePPOActor):
    ...
    @GPUMemoryLogger(role="dp actor", logger=logger)
    def update_sft_policy(self, data: DataProto):
        """Run a response-token SFT update on generated trajectories.

        This is used by the in-memory online RSFT trainer. The rollout engine
        stays alive in the same hybrid worker group, while this method updates
        the actor weights on verifier-accepted responses.
        """
        self.actor_module.train()
        ...
        sft_epochs = int(data.meta_info.get("sft_epochs", 1))
        mini_batch_size = int(data.meta_info.get("sft_mini_batch_size", self.config.ppo_mini_batch_size))
        micro_batch_size = int(
            data.meta_info.get("sft_micro_batch_size_per_gpu", self.config.ppo_micro_batch_size_per_gpu)
        )
        mini_batch_size = max(1, min(mini_batch_size, len(data)))
        micro_batch_size = max(1, min(micro_batch_size, mini_batch_size))

        metrics = {
            "actor/sft_loss": 0.0,
            "actor/sft_token_count": 0.0,
        }
        num_minibatches = 0
        ...
        for _ in range(sft_epochs):
            ...
            for mini_batch in mini_batches:
                ...
                mini_token_count = mini_batch.batch["response_mask"].sum().detach().clamp(min=1).to(get_device_id())
                self.actor_optimizer.zero_grad()
                ...
                for micro_batch in micro_batches:
                    ...
                    micro_token_count = response_mask.sum().detach().clamp(min=1)

                    outputs = self._forward_micro_batch(
                        model_inputs, temperature=temperature, calculate_entropy=False
                    )
                    log_prob = outputs["log_probs"]
                    micro_loss = -verl_F.masked_sum(log_prob, response_mask) / micro_token_count
                    loss_scale_factor = micro_token_count / mini_token_count
                    loss = micro_loss * loss_scale_factor
                    ...
                    metrics["actor/sft_loss"] += micro_loss.detach().item() * loss_scale_factor.detach().item()
                    metrics["actor/sft_token_count"] += micro_token_count.detach().item()

                grad_norm = self._optimizer_step()
                append_to_dict(metrics, {"actor/grad_norm": grad_norm.detach().item()})

        self.actor_optimizer.zero_grad()
        metrics["actor/sft_updates"] = float(num_minibatches)
        return metrics
