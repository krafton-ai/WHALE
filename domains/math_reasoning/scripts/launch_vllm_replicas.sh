...
#   MASTER_PORT_BASE default BASE_PORT + 20000
#                                      (each replica gets a distinct
...
MASTER_PORT_BASE="${MASTER_PORT_BASE:-$((BASE_PORT + 20000))}"
...
#
# --enable-chunked-prefill and --enable-prefix-caching mirror the current
# RSFT/GRPO rollout defaults. Keep MH standalone eval on the same vLLM prefill
# behavior as training rollout so harness selection is measured under the
# inference path used by subsequent training rounds.
#
...
EXTRA_ARGS="${EXTRA_ARGS:---enforce-eager --language-model-only --enable-chunked-prefill --enable-prefix-caching --gdn-prefill-backend triton --trust-remote-code}"
...
echo "[multi-vllm] MASTER_ADDR=$MASTER_ADDR MASTER_PORT_BASE=$MASTER_PORT_BASE"
...
rc=$?
set -e
echo "[multi-vllm] a vLLM replica exited with rc=$rc; terminating remaining replicas" >&2
exit "$rc"
