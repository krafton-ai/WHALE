#!/usr/bin/env python3
"""Agent-driven meta-harness loop for the ReTool math tool-use task.
...
Each run is isolated under runs/<run_name>/:
  harnesses/h0/harness.py   ← baseline copied from environments/retool/base_harness.py
...
CONFIG_PATH = EXPERIMENT_DIR / "config-retool.json"
BASE_ENV_PY = REPO_ROOT / "environments" / "retool" / "env.py"
BASELINE_HARNESS = Path(
    os.environ.get(
        "BASELINE_HARNESS_OVERRIDE",
        str(REPO_ROOT / "environments" / "retool" / "base_harness.py"),
    )
)
SKILL_DIR = EXPERIMENT_DIR / "skills" / "meta-harness-retool"
PROMPT_ONLY_SKILL_DIR = EXPERIMENT_DIR / "skills" / "meta-harness-retool-promptonly"
...
def append_evolution_summary(run_dir: Path, rows: list[dict]) -> None:
    ...
    with open(path, "a") as fh:
        for row in rows:
            key = (row.get("iteration"), row.get("harness"), row.get("status"))
            if key in seen:
                continue
...
def _prompt_only_normalized_dump(source: str) -> str:
    ...
    AST. Everything else (imports, helpers, class bodies, MAX_TURNS, ...)
    ...
    class _Mask(ast.NodeTransformer):
        def _mask_if_prompt(self, node, names):
            if names and set(names) <= _PROMPT_ONLY_NAMES:
                node.value = ast.copy_location(
                    ast.Constant(value="__PROMPT__"), node.value or node
                )
            return node
        ...
        def visit_Assign(self, node):
            ...
            return self._mask_if_prompt(node, names)
        ...
        def visit_AnnAssign(self, node):
            ...
            if (
                ...
                # Normalize to a plain Assign so `X: str | None = ...` and
                # `X = ...` compare equal for the prompt constants.
                return ast.Assign(
...
def validate_candidate(run_dir: Path, harness_name: str) -> None:
    """Check that harness.py exists and exposes CandidateEnv."""
    ...
    if "class CandidateEnv" not in content and "CandidateEnv =" not in content:
        raise ValueError(
            f"class CandidateEnv not found in {harness_path}. "
            "Proposer must define class CandidateEnv(ReToolEnv) or expose "
            "a CandidateEnv alias."
        )
    if PROMPT_ONLY:
...
def render_task_prompt(iteration: int, next_names: list[str]) -> str:
    ...
    return (
        f"Run iteration {iteration} of the ReTool harness evolution loop.\n\n"
        ...
        f"Follow the `meta-harness-retool` skill as your operating procedure."
    )
...
def _summarize_candidates(
    iteration: int, candidates: list[dict], results: dict, frontier: dict
) -> list[dict]:
    summary_rows = []
    for c in candidates:
        name = c["name"]
        ...
        row = {
            ...
            # Paper-style axis/components: persisted when the proposer provided
            # them (see SKILL.md), so cross-iteration analysis can see which
            # mechanism axes have been explored.
            "axis": c.get("axis", "?"),
        }
        if "components" in c:
...
def _iteration_dirs(logs_dir: Path) -> list[Path]:
    dirs = []
    for path in logs_dir.glob("iteration_*"):
        number = _iteration_number(path)
        if number is not None:
            dirs.append(path)
    return sorted(dirs, key=lambda p: _iteration_number(p) or 0)


def _find_pending_iteration_dir(logs_dir: Path, pending_eval: Path) -> Path | None:
    try:
        pending = json.loads(pending_eval.read_text())
    except (OSError, json.JSONDecodeError):
        return None
    iteration_dirs = list(reversed(_iteration_dirs(logs_dir)))
    for iteration_dir in iteration_dirs:
        if (iteration_dir / "comparison.json").exists():
            continue
        iter_pending = iteration_dir / "pending_eval.json"
        if iter_pending.exists():
            try:
                if json.loads(iter_pending.read_text()) == pending:
                    return iteration_dir
            except (OSError, json.JSONDecodeError):
                continue
        elif (iteration_dir / "proposer_prompt.txt").exists():
            # The job may have died after Claude wrote run_dir/pending_eval.json
            # but before the outer loop copied it into logs/iteration_NNN.
            names = [
                str(item.get("name", ""))
            ...
            if names and all(
                (iteration_dir.parents[1] / "harnesses" / name / "harness.py").exists()
                for name in names
            ):
                shutil.copy(pending_eval, iter_pending)
                return iteration_dir
    return None


def _pending_matches_completed_iteration(logs_dir: Path, pending_eval: Path) -> bool:
    try:
        pending = json.loads(pending_eval.read_text())
    except (OSError, json.JSONDecodeError):
        return False
    for iteration_dir in reversed(_iteration_dirs(logs_dir)):
        if not (iteration_dir / "comparison.json").exists():
            continue
        iter_pending = iteration_dir / "pending_eval.json"
        if not iter_pending.exists():
            continue
        try:
            if json.loads(iter_pending.read_text()) == pending:
                return True
...
def _recover_acceptance_from_comparisons(run_dir: Path, logs_dir: Path) -> str:
    """Repair accepted_harness.txt if death happened after comparison write."""
    for iteration_dir in reversed(_iteration_dirs(logs_dir)):
        ...
        try:
            ...
            continue
        frontier = comparison.get("frontier") or {}
        ...
        if current != resolved:
            ...
            print(
                f"{_ts()} {_yellow('recovered accepted harness')} "
                f"{current} -> {resolved} from {comparison_path}"
            )
        return resolved
    return get_accepted_harness(run_dir)


def _completed_iteration_numbers(logs_dir: Path) -> list[int]:
    nums = []
    for path in _iteration_dirs(logs_dir):
...
def _completed_through(logs_dir: Path) -> int:
    completed = 0
    for number in _completed_iteration_numbers(logs_dir):
...
def _next_iteration_target_reached(logs_dir: Path, target_iterations: int) -> bool:
    return _completed_through(logs_dir) >= target_iterations
...
def run_evolve(args: argparse.Namespace) -> None:
    ...
    target_iterations = max(int(args.iterations), 0)

    profiles = list(config["task_profiles"].keys())
    models = [get_model_short_name(m["model"]) for m in config["models"]]
    print(
        f"{_ts()} {_bold('Claude Meta-Harness (ReTool) — Full Harness Evolution')}  "
    ...
    # environments/retool/base_harness.py). The h0 success_rate is directly
    # comparable to the GRPO correctness reward because both use
    # math_dapo-style boxed-answer scoring.
    ...
    _recover_acceptance_from_comparisons(run_dir, logs_dir)

    # Resume an interrupted iteration after candidates were proposed but before
    # their benchmark/comparison finished. This is the common Slurm-timeout case:
    # run_dir/pending_eval.json and logs/iteration_NNN/pending_eval.json exist,
    # harnesses/hK are already written, but comparison.json is absent.
    if not args.fresh and pending_eval.exists():
        pending_iteration_dir = _find_pending_iteration_dir(logs_dir, pending_eval)
        if pending_iteration_dir is not None:
            iteration = _iteration_number(pending_iteration_dir) or 0
            current_harness = get_accepted_harness(run_dir)
            print(
                f"\n{_ts()} {_bold(f'Resuming pending iteration {iteration}')} "
            ...
            if valid_candidates and not args.propose_only:
                bench_start = time.time()
                print(
                    f"  {_ts()} {_cyan('benchmarking')} {len(valid_candidates)} "
                    f"pending candidate(s) across profiles/models/seeds"
                )
                ...
                summary_rows = _summarize_candidates(
                    iteration, valid_candidates, results, frontier
                )
                ...
                (pending_iteration_dir / "comparison.json").write_text(
                ...
                print("  " + _dim(f"resume timing: bench={_elapsed(bench_time)}"))
            elif valid_candidates and args.propose_only:
                print(
                    f"  {_ts()} {_yellow('propose-only: pending candidates validated only')}"
                )
            pending_eval.unlink(missing_ok=True)
            _recover_acceptance_from_comparisons(run_dir, logs_dir)
        elif _pending_matches_completed_iteration(logs_dir, pending_eval):
            pending_eval.unlink(missing_ok=True)
            print(
                f"{_ts()} {_yellow('cleared stale pending_eval.json')} "
                "after completed comparison recovery"
            )
        else:
            raise RuntimeError(
                f"Unmatched pending_eval.json in {run_dir}; refusing to overwrite "
                "possible partial proposer output"
            )
    ...
    es_best_iter = 0
    # -- Phase 1..N: evolution -----------------------------------------------
    while not _next_iteration_target_reached(logs_dir, target_iterations):
        if _interrupted:
            print("Interrupted.")
            break
        ...
        if not valid_candidates:
            ...
            append_evolution_summary(run_dir, invalid_rows)
            (iteration_dir / "comparison.json").write_text(
                json.dumps(
                    {
                        "iteration": iteration,
                        "baseline_harness": current_harness,
                        "candidates": invalid_rows,
                        "frontier": print_frontier(logs_dir, config)
    ...
    if _next_iteration_target_reached(logs_dir, target_iterations):
        completed = _completed_through(logs_dir)
        print(
            f"{_ts()} {_green('target iterations reached')} "
            f"completed={completed} target={target_iterations}"
        )


def _next_iteration_number(logs_dir: Path) -> int:
    # Iterations are complete only after their comparison.json is written.
    # If Slurm kills the job after creating logs/iteration_NNN but before
    # proposal/evaluation completes, relaunching must retry NNN instead of
    # skipping to the next number.
    return _completed_through(logs_dir) + 1
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=int,
        default=10,
        help="Total target number of evolution iterations for this run.",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--config",
        default=None,
        help="Path to config JSON (default: config-retool.json in script dir)",
    )
