...
_CORRECT_METRIC_KEYS = ("correct_answer", "correct_reward", "correct_answer_llm_judge")
...
class RoundRobinAsyncOpenAI:
    ...
    Used for meta-harness eval against N vLLM servers (one per GPU) when
    RETOOL_VLLM_URLS or SEARCH_R1_VLLM_URLS is set to a CSV of /v1 endpoints.
    """
...
def _make_client(
    ...
    Multi-URL behavior: if RETOOL_VLLM_URLS or SEARCH_R1_VLLM_URLS is set with
    > 1 entry, the wrapped instance is RoundRobinAsyncOpenAI which round-robins
    ...
    multi = os.environ.get("RETOOL_VLLM_URLS", "").strip()
    if not multi:
...
async def evaluate_harness_module(
    *,
    ...
    if max_tokens is not None:
        ...
        if hasattr(env, "set_max_total_completion_tokens"):
...
def run_sweep(
    ...
    *,
    ...
    By default up to 3 harness jobs run concurrently. eval.max_concurrent is
    interpreted as the total in-flight example budget across those harness
    jobs, so each concurrent job receives a fair share of that budget.
    Set eval.parallel_harness_evals=1 to preserve sequential harness
    evaluation.
    """
    ...
    total_max_concurrent = _cfg_int(eval_cfg.get("max_concurrent", 1), 1)
    if total_max_concurrent is None or total_max_concurrent < 1:
        total_max_concurrent = 1
    ...
    active_harness_evals = min(parallel_harness_evals, len(jobs)) if jobs else 1
    per_harness_max_concurrent = max(
        1,
        (total_max_concurrent + active_harness_evals - 1) // active_harness_evals,
    )
    if active_harness_evals > 1:
        print(
            "  [bench] total max_concurrent="
            f"{total_max_concurrent}; per-harness max_concurrent="
            f"{per_harness_max_concurrent} across {active_harness_evals} "
            "concurrent harness eval job(s)",
            flush=True,
        )
        jobs = [
            (desc, {**kwargs, "max_concurrent": per_harness_max_concurrent})
            for desc, kwargs in jobs
        ]

    if parallel_harness_evals <= 1 or len(jobs) <= 1:
        return [
            _run_eval_job_worker((desc, kwargs, eval_retries))
            for desc, kwargs in jobs
        ]
