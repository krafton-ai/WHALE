...
def compute_data_metrics(batch: DataProto, use_critic: bool = True) -> dict[str, Any]:
    ...
    assistant_generated_tokenized = response_mask.sum(-1).float()
    tool_user_observation_tokenized = torch.clamp(response_length - assistant_generated_tokenized, min=0.0)
    ...
    metrics = {
        ...
        **(
        ...
        .detach()
        .item(),
        # Explicit aliases for multi-turn agent rollouts. In multi-turn mode,
        # verl's response region includes assistant generations plus tool/user
        # observations appended after the initial prompt; response_mask marks
        # the assistant-generated subset.
        "tokens/response_region_total_includes_assistant_and_tool_user/mean": torch.mean(response_length)
        .detach()
        .item(),
        "tokens/response_region_total_includes_assistant_and_tool_user/max": torch.max(response_length)
        .detach()
        .item(),
        "tokens/response_region_total_includes_assistant_and_tool_user/min": torch.min(response_length)
        .detach()
        .item(),
        "tokens/assistant_generated_tokenized/mean": torch.mean(assistant_generated_tokenized).detach().item(),
        "tokens/assistant_generated_tokenized/max": torch.max(assistant_generated_tokenized).detach().item(),
        "tokens/assistant_generated_tokenized/min": torch.min(assistant_generated_tokenized).detach().item(),
        "tokens/tool_user_observation_tokenized/mean": torch.mean(tool_user_observation_tokenized).detach().item(),
        "tokens/tool_user_observation_tokenized/max": torch.max(tool_user_observation_tokenized).detach().item(),
        "tokens/tool_user_observation_tokenized/min": torch.min(tool_user_observation_tokenized).detach().item(),
    ...
    for source_key, metric_prefix in {
        "assistant_generated_tokens_usage": "tokens/assistant_generated_usage_from_vllm",
        "assistant_generated_tokens_tokenized": "tokens/assistant_generated_tokenized_from_agent",
        "response_region_tokens_total": "tokens/response_region_total_from_agent",
        "tool_user_observation_tokens_tokenized": "tokens/tool_user_observation_tokenized_from_agent",
        "prompt_prefix_tokens": "tokens/prompt_prefix_from_agent",
        "max_turns_effective": "chess_puzzle/max_turns_effective",
    }.items():
        if source_key not in batch.non_tensor_batch:
            continue
        values = np.asarray(batch.non_tensor_batch[source_key], dtype=np.float32)
        if values.size == 0:
            continue
        metrics[f"{metric_prefix}/mean"] = float(values.mean())
        metrics[f"{metric_prefix}/max"] = float(values.max())
        metrics[f"{metric_prefix}/min"] = float(values.min())
...
def compute_throughout_metrics(batch: DataProto, timing_raw: dict[str, float], n_gpus: int) -> dict[str, Any]:
    ...
    Returns:
        A dictionary containing:
            ...
            - perf/rollout_gen_total_tps: Prompt + response tokens per second during rollout generation
            - perf/rollout_gen_total_tps_per_gpu: Prompt + response tokens per second per GPU during rollout generation
            - perf/rollout_gen_response_tps: Response tokens per second during rollout generation
            - perf/rollout_gen_response_tps_per_gpu: Response tokens per second per GPU during rollout generation
    ...
    num_prompt_tokens = torch.sum(response_info["prompt_length"]).item()
    num_response_tokens = torch.sum(response_info["response_length"]).item()
    rollout_num_tokens = num_prompt_tokens + num_response_tokens

    rollout_metrics = {}
    gen_time = timing_raw.get("gen")
    if gen_time is not None and gen_time > 0:
        rollout_metrics = {
            "perf/rollout_gen_prompt_tokens": num_prompt_tokens,
            "perf/rollout_gen_response_tokens": num_response_tokens,
            "perf/rollout_gen_total_tokens": rollout_num_tokens,
            "perf/rollout_gen_total_tps": rollout_num_tokens / gen_time,
            "perf/rollout_gen_total_tps_per_gpu": rollout_num_tokens / (gen_time * n_gpus),
            "perf/rollout_gen_response_tps": num_response_tokens / gen_time,
            "perf/rollout_gen_response_tps_per_gpu": num_response_tokens / (gen_time * n_gpus),
        }
...
def bootstrap_metric(
    ...
    bootstrap_idxs = np.random.RandomState(seed).choice(n_data, size=(n_bootstrap, subset_size), replace=True)

    if (
        len(reduce_fns) == 2
        and reduce_fns[0] is np.max
        and reduce_fns[1] is np.min
        and np.issubdtype(data_np.dtype, np.number)
    ):
        samples = data_np[bootstrap_idxs]
        metric_results = np.stack((np.max(samples, axis=1), np.min(samples, axis=1)))
        return [
            (float(np.mean(metric_results[fn_idx])), float(np.std(metric_results[fn_idx])))
            for fn_idx in range(len(reduce_fns))
        ]

    if data_np.dtype != object:
        data_np = data_np.astype(object)
        ...
        for uid, var2vals in uid2var2vals.items():
            ...
            for var_name, var_vals in var2vals.items():
                ...
                if n_resps > 1:
                    ...
                    for n in ns:
                        # compute best/worst metrics
                        cache_key = ("best_worst", n, tuple(var_vals))
                        if cache_key not in bootstrap_cache:
                            bootstrap_cache[cache_key] = bootstrap_metric(
                                data=var_vals,
                                subset_size=n,
                        ...
                        (bon_mean, bon_std), (won_mean, won_std) = bootstrap_cache[cache_key]
