...
class ToolAgentLoop(AgentLoopBase):
    ...
    async def run(self, sampling_params: dict[str, Any], **kwargs) -> AgentLoopOutput:
        ...
        extra_fields = dict(agent_data.extra_fields or {})
        # harness_state is internal rollout state for CandidateEnv parity.
        # It is a nested per-trajectory dict and must not be returned in
        # AgentLoopOutput, because DataProto.concat expects non-tensor fields
        # to have batch-size-aligned lengths across all trajectories.
        extra_fields.pop("harness_state", None)
    ...
    async def _handle_generating_state(
        ...
        try:
            from verl.utils.meta_harness_hook import (
                apply_candidate_get_model_response_sampling,
            )

            turn_sampling_params = await apply_candidate_get_model_response_sampling(
                agent_data,
                turn_sampling_params or sampling_params,
                max_turns=self.max_assistant_turns,
            )
            if "tool_choice" in turn_sampling_params:
                # Candidate MH eval may run through an OpenAI-compatible chat
                # endpoint, where tool_choice="required" is a request-level
                # constraint. This verl path uses token-level vLLM
                # SamplingParams, which does not accept that OpenAI-only key.
                # Record that the harness asked for it, then remove it so the
                # common GRPO path does not fail before generation.
                agent_data.metrics["harness_tool_choice_requested"] = 1
                turn_sampling_params = dict(turn_sampling_params)
                turn_sampling_params.pop("tool_choice", None)
        except Exception as exc:  # pragma: no cover — defensive
            logger.warning(
                "meta-harness get_model_response sampling hook skipped: %s", exc
            )
        ...
        with simple_timer("generate_sequences", agent_data.metrics):
            output: TokenOutput = await self.server_manager.generate(
                ...
                sampling_params=dict(turn_sampling_params or sampling_params),
        ...
        # max_turns_reached, dispatched elsewhere). This must happen AFTER
        # tool-call extraction so hooks that inspect the latest completion see
        # the same `tool_calls` field they would see under meta-harness eval.
        try:
            from verl.utils.meta_harness_hook import check_candidate_extra_stop_hooks

            if await check_candidate_extra_stop_hooks(
                agent_data,
