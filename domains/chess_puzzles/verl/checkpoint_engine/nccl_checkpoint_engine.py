...
def _ray_collective_timeout_s() -> int | None:
    raw = os.getenv("VERL_RAY_COLLECTIVE_NCCL_ID_TIMEOUT_S") or os.getenv("RAY_COLLECTIVE_NCCL_ID_TIMEOUT_S")
    if not raw:
        return None
    try:
        value = int(raw)
    except ValueError:
        logger.warning("Ignoring invalid Ray collective NCCL timeout: %s", raw)
        return None
    return value if value > 0 else None


def _patch_ray_collective_timeout() -> None:
    timeout_s = _ray_collective_timeout_s()
    if timeout_s is None:
        return
    try:
        import ray.util.collective.collective_group.nccl_collective_group as nccl_group
    except Exception as exc:
        logger.warning("Failed to patch Ray collective NCCL timeout: %s", exc)
        return

    rendezvous_cls = nccl_group.Rendezvous
    if getattr(rendezvous_cls, "_verl_timeout_patch_s", None) == timeout_s:
        return

    orig_meet = getattr(rendezvous_cls.meet, "_verl_orig", rendezvous_cls.meet)
    orig_get_nccl_id = getattr(rendezvous_cls.get_nccl_id, "_verl_orig", rendezvous_cls.get_nccl_id)

    def meet(self, timeout_s_override=None):
        return orig_meet(self, timeout_s if timeout_s_override is None else timeout_s_override)

    def get_nccl_id(self, timeout_s_override=None):
        return orig_get_nccl_id(self, timeout_s if timeout_s_override is None else timeout_s_override)

    meet._verl_orig = orig_meet
    get_nccl_id._verl_orig = orig_get_nccl_id
    rendezvous_cls.meet = meet
    rendezvous_cls.get_nccl_id = get_nccl_id
    rendezvous_cls._verl_timeout_patch_s = timeout_s
    logger.warning("Patched Ray collective NCCL rendezvous timeout to %ss", timeout_s)
