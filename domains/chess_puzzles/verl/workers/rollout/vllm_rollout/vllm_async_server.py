...
_SAMPLING_PARAM_KEYS = set(inspect.signature(SamplingParams).parameters)
...
def _normalize_sampling_params_for_vllm(sampling_params: dict[str, Any]) -> dict[str, Any]:
    """Drop OpenAI-server-only request fields before building vLLM SamplingParams."""
    extra_body = sampling_params.pop("extra_body", None)
    if isinstance(extra_body, dict):
        for key, value in extra_body.items():
            if key in _SAMPLING_PARAM_KEYS and key not in sampling_params:
...
class vLLMHttpServer:
    ...
    async def generate(
        ...
        sampling_params = _normalize_sampling_params_for_vllm(sampling_params)
