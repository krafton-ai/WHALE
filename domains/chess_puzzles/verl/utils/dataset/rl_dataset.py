...
class RLHFDataset(Dataset):
    ...
    def _build_messages(self, example: dict):
        ...
        # Meta-harness hook: when HARNESS_PATH points to a candidate, replace the
        # system message content with CandidateEnv.SYSTEM_PROMPT and the user
        # message content with CandidateEnv.USER_PROMPT_TEMPLATE.format(question=...).
        # No-op if HARNESS_PATH is unset. See verl/utils/meta_harness_hook.py.
        try:
            from verl.utils.meta_harness_hook import apply_prompt_overrides

            messages = apply_prompt_overrides(messages, example)
        except Exception as exc:  # pragma: no cover — defensive: never crash data loading
            logger.warning("meta-harness prompt override skipped: %s", exc)
